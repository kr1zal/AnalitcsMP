...
| VPS | Beget 83.222.16.15, Ubuntu 24.04 | SSH: `ssh -i ~/.ssh/id_ed25519 -p 2222 root@83.222.16.15` (key-only, порт 2222) |
|-----|-----------------------------------|------------------------------------------------------|
...
43. **UE Ozon = delivery-based (миграция 036, 01.03.2026):** RPC `get_ozon_ue_delivered` фильтрует по `order_date IN period AND delivery_date <= period_end` (совпадает с ЛК). Revenue и sales_count из finance-settled "Выручка" записей (НЕ из mp_sales). COGS = purchase_price × delivered_settled_count. Posting_number JOIN: exact + LIKE prefix (base posting без item suffix). Формула Ozon: `profit = SUM(mp_costs_details.amount) - daily_storage - COGS`. WB = proportional payout из costs-tree (без изменений)
...
54. **Ozon UE storage-only fix (09.03.2026):** Storage loop в dashboard.py ТОЛЬКО модифицирует СУЩЕСТВУЮЩИЕ записи в `ozon_order_date_by_product` (products с finance records). НИКОГДА не создавать новые записи `{"payout": 0.0, "revenue": 0.0}` для storage-only products — это триггерит delivery-based path вместо storage-only safety check (line 964). Storage-only products: `net_profit = -storage_cost` (safety check, sales_count==0).
