-- Migration 039: Fix ghost orders in dashboard "Заказы" card
--
-- Problem: WB reportDetailByPeriod creates ghost orders (price=0) in mp_orders.
-- Dashboard counts them: 19 orders instead of 7.
-- Also SUM(price) uses catalog price, not sale_price (after SPP).
--
-- Fix: Add "AND price > 0" + use COALESCE(sale_price, price) for sum.
-- Minimal change: only the orders query (lines 57-67), rest untouched.
-- Safety: Ozon has no ghost orders (price always > 0).

-- Recreate with the fix (same signature, same return type)
...
BEGIN
  -- FIX 039: Exclude ghost orders (price=0) + use sale_price when available
  SELECT
    COALESCE(COUNT(DISTINCT order_id), 0),
    COALESCE(SUM(COALESCE(sale_price, price)), 0)
