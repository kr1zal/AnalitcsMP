-- =============================================
-- 029: Fix RPC profit axis — settlement-based поля из mp_costs_details
--
-- ПРОБЛЕМА: RPC get_dashboard_summary вычисляет net_profit через:
--   revenue (mp_sales, order-based) - total_costs (mp_costs, settlement-based) - purchase - ads
--   Это мешает оси: order-based revenue + settlement-based costs = ложная прибыль.
--   Пример: Ozon 01.02-27.02 показывает -2,919 вместо реальных -7,022.
--
-- РЕШЕНИЕ: Добавить settlement-based поля из mp_costs_details:
--   - settled_revenue: SUM(amount) WHERE category='Продажи' (финализированная выручка)
--   - settled_purchase: SUM(purchase_price * settled_qty) из mp_costs (settlement-based)
--   - settled_payout: SUM(all amounts) из mp_costs_details (= costs-tree total_accrued)
--   - settled_profit: settled_payout - settled_purchase - ad_cost
--
-- Frontend будет использовать settled_* поля для карточек Себестоимость и Прибыль,
-- вместо зависимости от UE endpoint.
-- =============================================
...
DECLARE
  ...
  -- NEW: settlement-based fields from mp_costs_details
...
BEGIN
  ...
  -- NEW: Settlement-based revenue (из mp_costs_details, category='Продажи')
  -- Финализированная выручка из финансового отчёта МП
  ...
  -- NEW: Settlement-based payout (ВСЕ категории из mp_costs_details = costs-tree total_accrued)
  SELECT COALESCE(SUM(amount), 0)
  ...
  -- NEW: Settlement-based purchase (purchase_price * settled_qty из mp_costs)
  -- Для Ozon: settled_qty = количество единиц по дате расчёта
  -- Для WB: settled_qty может быть 0, fallback на sales_count через GREATEST
  SELECT COALESCE(SUM(
...
-- Теперь settled_* поля автоматически доступны через v_current_summary->'summary'
...
BEGIN
  ...
  RETURN json_build_object(
    ...
    'previous_period', json_build_object(
      ...
      -- NEW: prev period settlement-based
