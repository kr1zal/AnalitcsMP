...
- costsTreeRatio использовал ЧИСТЫЕ продажи (без credits) — **costsTreeRatio удалён 19.02.2026**
...
| **Прибыль** | `total_accrued - purchase - ad` | Чистая прибыль (с 19.02.2026, RAW purchase) |

### costsTreeRatio (УСТАРЕЛО — удалено 19.02.2026)
```
// ИСТОРИЧЕСКАЯ формула (до 19.02.2026):
...
- Credits НЕ входили в ratio — они компенсация от МП, не от продаж
- **Удалено 19.02.2026.** Новая формула: `profit = payout - purchase - ads` (purchase RAW)

### Per-marketplace profit (актуально с 19.02.2026)
```
share = pureSales_mp / totalPureSales (OZON+WB)
profit_mp = payout_mp - purchase×share - ad×share  // purchase RAW
```
...
3. Себестоимость RAW: `purchase = price × qty` (costsTreeRatio удалён 19.02.2026)
...
| **Прибыль** | `total_accrued - purchase×share - ad×share` (purchase RAW с 19.02.2026) | DashboardPage |

### Per-MP profit (актуально с 19.02.2026)
```
ozonPureSales = tree["Продажи"].amount (без credits — у OZON их нет)
share = ozonPureSales / (ozonPureSales + wbPureSales)
profit_ozon = ozon_total_accrued - purchase × share - ad × share  // purchase RAW
```
...
4. Себестоимость RAW: `purchase = price × qty` (costsTreeRatio удалён 19.02.2026)
