...
| Webhook URL | https://analitics.bixirun.ru/api/v1/subscription/webhook |
...
| `backend/app/api/v1/payment.py` | 3 эндпоинта: /subscription/upgrade, /webhook, /cancel |
...
| Файл | Назначение |
|------|-----------|
| `frontend/src/services/api.ts` | `paymentApi.upgrade(plan)`, `paymentApi.cancel()` |
| `frontend/src/hooks/useSubscription.ts` | `useUpgrade()`, `useCancelSubscription()` хуки |
| `frontend/src/components/Settings/SubscriptionCard.tsx` | Кнопка "Подключить Pro" + "Отменить автопродление" |
...
1. Применить миграцию 012 в Supabase SQL Editor
