...
| Слайдер | Чистый React + CSS | — | ProductShowcase на Landing (без Swiper) |
...
**Файл:** `pages/LandingPage.tsx` (~2000 строк)

Rich landing: Swiper-карусель, scroll-reveal, Canvas MatrixRain, счетчики, pricing.

**Структура секций:**
- **NavBar** — sticky, backdrop-blur при скролле, кликабельный логотип (scroll-to-top), `NAV_ITEMS` const, clean gaps (hover:bg-gray-50), hamburger 44px touch target
- **HeroSection** — текстовый hero без скриншота. H1 "Прозрачная аналитика для маркетплейсов" (primary USP), badge "WB + Ozon в одном дашборде" (differentiator), подзаголовок с secondary USP "Собери свой дашборд из виджетов". CTA + trust inline. MatrixRain canvas фон
- **TrustBar** — бесконечный marquee с API/security бейджами
- **DashboardCarousel** — Swiper, 3 слайда: Дашборд, Юнит-экономика, Остатки. ScreenshotSlide с browser chrome, WebP + PNG fallback
- **SocialProofSection** — социальное доказательство
- **FeaturesSection** — 6 фич с spotlight hover
- **PricingSection** — grid-cols-2, Free/Pro
- **SecuritySection** — безопасность данных
- **FAQSection** — аккордеон с ARIA (aria-expanded, aria-controls)
- **StatsBar** — анимированные счётчики
