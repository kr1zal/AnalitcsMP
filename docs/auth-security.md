...
| Feature | Free | Pro (1 490 руб/мес) | Business (2990 руб/мес) |
|---------|------|----|----|
| `dashboard` | Да | Да | Да |
| `costs_tree_basic` | Да | Да | Да |
| `costs_tree_details` | Да | Да | Да |
| `unit_economics` | -- | Да | Да |
| `ads_page` | -- | Да | Да |
| `pdf_export` | -- | Да | Да |
| `period_comparison` | Да | Да | Да |
| `fbs_analytics` | -- | Да | Да |
| `order_monitor` | -- | -- | Да |
| `api_access` | -- | -- | Да |
...
|----------|------|-----|----------|
| Макс. SKU | 15 | 50 | Без лимита |
| Маркетплейсы | WB + Ozon | WB + Ozon | WB + Ozon |
| Автосинхронизация | 2 раза/день | Каждые 6ч | Каждые 2ч |
| Ручная синхронизация | 1/день | 3/день | 3/день |
...
| Telegram | Сводка 1x/день | Полный (алерты, AI) | Полный (алерты, AI) |
...
```python
PLANS = {
    "free":     { "name": "Free",     "price_rub": 0,    "max_sku": 15,   ... },
    "pro":      { "name": "Pro",      "price_rub": 1490, "max_sku": 50,   ... },
