...
## 2026‑01‑30 — Mobile: боковая плашка навигации вместо header

### Цели

- Убрать верхнее меню на мобиле; заменить на фиксированную боковую плашку справа.
- Удобство для правой руки: плашка на 25% от верха экрана (зона большого пальца).
- В выехавшей панели сразу 4 ссылки (без дополнительного шага «бутерброд»).

### Frontend

- **Layout.tsx**
  - **Desktop:** без изменений — header (логотип + горизонтальное меню), sticky.
  - **Mobile:** header не рендерится; контент сразу под верхним краем.
  - Фиксированная плашка справа: «язычок» 12px, позиция `top: 25%`, по тапу открывает панель (выезд влево, 280px / 85vw). В панели сразу 4 пункта: Дашборд, Unit-экономика, Реклама, Синхронизация.
  - Закрытие: overlay, ESC, переход по ссылке; блокировка scroll при открытой панели.
- **index.css**
  - `.nav-tab-strip` — градиент indigo→violet, анимация переливания 5s; hover — ширина 12px→16px, усиленная тень.
  - `@media (prefers-reduced-motion: reduce)` — анимация отключена.

### Документация

- Обновлены: CLAUDE.md, frontend/README.md, frontend/DESIGN_REFERENCE.md, promt.md — описание Layout (mobile: плашка + панель; desktop: header + nav).
